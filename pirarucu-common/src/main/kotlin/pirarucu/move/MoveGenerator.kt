...
object MoveGenerator {
    ...
    fun legalMoves(board: Board, moveList: MoveList) {
        ...
        when {
            ...
            Bitboard.oneElement(checkBitboard) -> {
                ...
                val betweenBitboard = BitboardMove.BETWEEN_BITBOARD[board.basicEvalInfo.kingSquare[ourColor]][square]
    ...
    private fun legalPawnMoves(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        while (tmpPieces != 0L) {
            ...
            if (bitboard != 0L) {
                bitboard = bitboard or BitboardMove.DOUBLE_PAWN_MOVES[ourColor][fromSquare] and emptyBitboard
            }
            ...
            while (bitboard != 0L) {
                ...
                when {
                    Bitboard.PROMOTION_BITBOARD and toBitboard != 0L -> {
                        moveList.addMove(Move.createPromotionMove(fromSquare, toSquare, MoveType.TYPE_PROMOTION_KNIGHT))
                        moveList.addMove(Move.createPromotionMove(fromSquare, toSquare, MoveType.TYPE_PROMOTION_BISHOP))
                        moveList.addMove(Move.createPromotionMove(fromSquare, toSquare, MoveType.TYPE_PROMOTION_ROOK))
                        moveList.addMove(Move.createPromotionMove(fromSquare, toSquare, MoveType.TYPE_PROMOTION_QUEEN))
                    }
    ...
    private fun legalBishopMoves(board: Board, moveList: MoveList, maskBitboard: Long) {
        val ourColor = board.colorToMove
        val kingSquare = board.basicEvalInfo.kingSquare[ourColor]
        var pieces = board.pieceBitboard[ourColor][Piece.BISHOP] or board.pieceBitboard[ourColor][Piece.QUEEN]
        var tmpPieces = pieces
        while (tmpPieces != 0L) {
            val fromSquare = Square.getSquare(tmpPieces)
            val fromBitboard = Bitboard.getBitboard(fromSquare)
            var bitboard = BitboardMove.bishopMoves(fromSquare, board.gameBitboard) and maskBitboard
            if (fromBitboard and board.basicEvalInfo.pinnedBitboard[ourColor] != 0L) {
                bitboard = bitboard and BitboardMove.PINNED_MOVE_MASK[kingSquare][fromSquare]
            }
            ...
            while (bitboard != 0L) {
                val toSquare = Square.getSquare(bitboard)
                moveList.addMove(Move.createMove(fromSquare, toSquare, board.pieceTypeBoard[fromSquare]))
                bitboard = bitboard and bitboard - 1
            }
            tmpPieces = tmpPieces and tmpPieces - 1
        }
    ...
    private fun legalRookMoves(board: Board, moveList: MoveList, maskBitboard: Long) {
        val ourColor = board.colorToMove
        val kingSquare = board.basicEvalInfo.kingSquare[ourColor]
        var pieces = board.pieceBitboard[ourColor][Piece.ROOK] or board.pieceBitboard[ourColor][Piece.QUEEN]
        var tmpPieces = pieces
        while (tmpPieces != 0L) {
            val fromSquare = Square.getSquare(tmpPieces)
            val fromBitboard = Bitboard.getBitboard(fromSquare)
            var bitboard = BitboardMove.rookMoves(fromSquare, board.gameBitboard) and maskBitboard
            if (fromBitboard and board.basicEvalInfo.pinnedBitboard[ourColor] != 0L) {
                bitboard = bitboard and BitboardMove.PINNED_MOVE_MASK[kingSquare][fromSquare]
            }
    ...
    private fun legalKingMoves(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        while (moves != 0L) {
            ...
            if (!squareUnderAttack(toSquare, ourColor, board.pieceBitboard[theirColor], gameBitboard)) {
    ...
    private fun castlingMoves(board: Board, moveList: MoveList) {
        val ourColor = board.colorToMove
        var castlingIndexes = CastlingRights.filterCastlingRight(ourColor, board.castlingRights)
        val kingSquare = board.basicEvalInfo.kingSquare[ourColor]
        while (castlingIndexes != 0) {
            val castlingIndex = Square.getSquare(castlingIndexes)
            val kingTo = CastlingRights.KING_FINAL_SQUARE[castlingIndex]
            val kingPath = BitboardMove.BETWEEN_BITBOARD[kingSquare][kingTo] or Bitboard.getBitboard(kingTo)
            ...
            if ((kingPath or rookPath) and board.gameBitboard == 0L) {
                ...
                if (!pathUnderAttack(kingPath, ourColor, board.pieceBitboard[theirColor], board.gameBitboard)) {
    ...
    private fun legalPawnEPCapture(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        if (epSquare != Square.NONE) {
            var epPawnBitboard = BitboardMove.PAWN_MOVES[theirColor][epSquare] and maskBitboard
            if (epPawnBitboard != 0L) {
                ...
                var tmpPieces = board.pieceBitboard[ourColor][Piece.PAWN] and BitboardMove.PAWN_ATTACKS[theirColor][epSquare]
                while (tmpPieces != 0L) {
                    val fromSquare = Square.getSquare(tmpPieces)
                    val fromBitboard = Bitboard.getBitboard(fromSquare)
                    val tmpBitboard = gameBitboard xor fromBitboard xor Bitboard.getBitboard(epSquare)
    ...
    private fun legalPawnAttacks(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        while (tmpPieces != 0L) {
            ...
            while (bitboard != 0L) {
                ...
                when {
                    Bitboard.PROMOTION_BITBOARD and toBitboard != 0L -> {
                        moveList.addMove(Move.createPromotionAttack(fromSquare, toSquare, board.pieceTypeBoard[toSquare], MoveType.TYPE_PROMOTION_KNIGHT))
                        moveList.addMove(Move.createPromotionAttack(fromSquare, toSquare, board.pieceTypeBoard[toSquare], MoveType.TYPE_PROMOTION_BISHOP))
                        moveList.addMove(Move.createPromotionAttack(fromSquare, toSquare, board.pieceTypeBoard[toSquare], MoveType.TYPE_PROMOTION_ROOK))
                        moveList.addMove(Move.createPromotionAttack(fromSquare, toSquare, board.pieceTypeBoard[toSquare], MoveType.TYPE_PROMOTION_QUEEN))
                    }
                    else -> moveList.addMove(Move.createAttackMove(fromSquare, toSquare, Piece.PAWN, board.pieceTypeBoard[toSquare]))
                }
                bitboard = bitboard and bitboard - 1
            }
            tmpPieces = tmpPieces and tmpPieces - 1
    ...
    private fun legalKnightAttacks(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        while (tmpPieces != 0L) {
            ...
            while (bitboard != 0L) {
                val toSquare = Square.getSquare(bitboard)
                moveList.addMove(Move.createAttackMove(fromSquare, toSquare, Piece.KNIGHT, board.pieceTypeBoard[toSquare]))
                bitboard = bitboard and bitboard - 1
            }
            tmpPieces = tmpPieces and tmpPieces - 1
        }
    ...
    private fun legalBishopAttacks(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        while (tmpPieces != 0L) {
            ...
            while (bitboard != 0L) {
                val toSquare = Square.getSquare(bitboard)
                moveList.addMove(Move.createAttackMove(fromSquare, toSquare, board.pieceTypeBoard[fromSquare], board.pieceTypeBoard[toSquare]))
                bitboard = bitboard and bitboard - 1
            }
            tmpPieces = tmpPieces and tmpPieces - 1
        }
    ...
    private fun legalKingAttacks(board: Board, moveList: MoveList, maskBitboard: Long) {
        ...
        while (moves != 0L) {
            val toSquare = Square.getSquare(moves)
            if (!squareUnderAttack(toSquare, ourColor, board.pieceBitboard[theirColor], gameBitboard)) {
                moveList.addMove(Move.createAttackMove(fromSquare, toSquare, Piece.KING, board.pieceTypeBoard[toSquare]))
            }
            moves = moves and moves - 1
        }
    }

    private fun pathUnderAttack(path: Long, ourColor: Int, theirPieceBitboard: LongArray, gameBitboard: Long): Boolean {
        var tmpPath = path
        while (tmpPath != 0L) {
    ...
    private fun squareUnderAttack(square: Int, ourColor: Int, theirPieceBitboard: LongArray, gameBitboard: Long): Boolean {
        val pawns = theirPieceBitboard[Piece.PAWN] and gameBitboard
        val knights = theirPieceBitboard[Piece.KNIGHT] and gameBitboard
        val bishops = (theirPieceBitboard[Piece.BISHOP] or theirPieceBitboard[Piece.QUEEN]) and gameBitboard
        val rooks = (theirPieceBitboard[Piece.ROOK] or theirPieceBitboard[Piece.QUEEN]) and gameBitboard
